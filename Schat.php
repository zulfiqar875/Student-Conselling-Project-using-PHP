        ...
        <html lang="en">
            ...
            <body>
                ...
                <div class="dashboard pt-5 pl-5 pr-5 pb-0 text-center">
                    <h3 class="text-info">Chat With Student: <b class="text-danger"><?php     
                                                                    $cid; 
                                                                    $run = mysqli_query($con, "SELECT * FROM `student` WHERE id = $sid");
                                                                    $data = mysqli_fetch_assoc($run);
                                                                    echo $data['fname'];   ?></b>
                ...
                <div class="addcounsellor p-5">
                    <div class="box">   
                        <?php 
                            $con=mysqli_connect('localhost','root','','student');
                            $query="SELECT * FROM `chat` WHERE sid=$sid";
                        ...
                        <form action="" method="post">  
                            ...
                            <input type="hidden" name="cid" value="<?php echo $user; ?>" class="form-control" id="">
                            <input type="hidden" name="sid" value="<?php echo $sid; ?>" class="form-control" id="">
...
<?php
    ...
    if(isset($_POST['chat']))
    {
        ...
        $run=mysqli_query($con,"INSERT INTO `chat`(`cid`,`sid`,`question`,`answer`,`status`) "
            . "VALUES ('$cid','$sid','$msg','Counsellor','1')");
            // echo '<script>alert("Registartion Completed")</script>';
        if($run)
        {
